...
public class AdminReservationsFeatureSteps {
    ...
    public void the_house_will_be_accepted() {
        ...
        for(Apartment apartment : adminMainPageHandler.apartments){
            if(apartment.getHouseId() == adminMainPageHandler.houseIDTest1) {notFound = false; break;}
        }
        assertTrue(notFound);
    }
    ...
    public void the_house_will_be_rejected() {
        ...
        boolean notFound = true;
        for(Apartment apartment : adminMainPageHandler.apartments){
            if(apartment.getHouseId() == adminMainPageHandler.houseIDTest2) {notFound = false; break;}
        }
        assertTrue(notFound);
    }
}
